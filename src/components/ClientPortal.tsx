...
function formatAssessment(summary: string) {
  if (!summary) {
    return {
      paragraphs: ['No assessment narrative is currently available for this record.']
    };
  }
  ...
  return {
    paragraphs: paragraphs.length > 0 ? paragraphs : [cleaned]
  };
}
...
function buildNetwork(record: Record): {
  ...
  addNodes(record.affiliations, 'affiliation', 'affiliation');
  addNodes(record.rivalries, 'rivalry', 'rivalry');
  addNodes(record.linked_events, 'event', 'event');
...
export default function ClientPortal({ onLogout }: ClientPortalProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/client/login', {
        ...
        body: JSON.stringify({
          access_key: accessKeyInput.toUpperCase()
        })
      });
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-[#8f9bad] text-sm">
              Enter your agency access key to view your specific intelligence feed.
            </p>
          </div>
  ...
  const assessment = selectedRecord
    ? formatAssessment(selectedRecord.summary)
    : null;
  ...
  const getNodePosition = (index: number, total: number) => {
    ...
    const angle =
      ((index - 1) / Math.max(total - 1, 1)) * Math.PI * 2 -
      Math.PI / 2;
        ...
        <div className="flex items-end justify-between mb-[18px]">
          <div className="brand">
            <div className="flex items-center gap-3">
              <Shield size={22} className="text-[#5b8def]" />
                ...
                >
                  ...
                  <div className="text-center max-w-lg">
                    ...
                    <p className="text-[12px] text-[#8f9bad] mt-2 leading-[1.7]">
                      Search the authorized intelligence feed using a name,
                      entity type, location, affiliation, rivalry, tag or
                      linked event.
                    </p>
